...
class WGANGP():
    ...
    , n_critic = 5
    , using_generator = False, save_model_flag = True):
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            if epoch % print_every_n_batches == 0:
                self.sample_images(run_folder, self.plot_size[0], self.plot_size[1])
                
                if save_model_flag:
                    self.save_model(run_folder)
                    self.model.save_weights(os.path.join(run_folder, 'weights/weights-%d.h5' % (epoch)))   
                    self.model.save_weights(os.path.join(run_folder, 'weights/weights.h5'))
                    #self.model.save_weights(os.path.join(run_folder, 'weights/weights.ckpt'))
                    
                
            self.epoch+=1


    def sample_images(self, run_folder, r=5, c=5):
